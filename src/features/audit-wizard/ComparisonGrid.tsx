      ...
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        ...
        <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900/70">
          {allNames.map((name) => {
            ...
            return (
              <tr key={name} className={changed ? 'bg-amber-50 dark:bg-amber-500/10' : ''}>
                <td className="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-100">{displayName}</td>
                ...
                <td className="px-4 py-3 text-sm">
                  {changed ? (
                    <span className="font-medium text-amber-700 dark:text-amber-300">Yes</span>
                  ) : (
